...
const PASSWORD = z.string().regex(/^\d{4,12}$/u).parse(process.env["DOORLOCK_PASSWORD"])
...
test("runs the complete authenticated production medical and failure smoke", async ({
  ...
  record("ordinary", ordinaryPrompt, "medium", ordinary.answer, ordinary.sourceHrefs, ordinary.rawStream)
  ...
  for (const effort of EFFORTS) {
    await resetConversation(page)
    await page.getByRole("combobox", { name: "추론 강도" }).selectOption(effort)
    const prompt = "산후 회복기에 체온을 확인할 때 일반적으로 기록해 둘 정보를 근거와 함께 짧게 알려 주세요."
  ...
  await resetConversation(page)
  await page.getByRole("combobox", { name: "추론 강도" }).selectOption("medium")
  ...
  await page.getByRole("textbox", { name: "의료 질문" }).fill(
    "이 사진만으로 확정하지 말고 관찰 가능한 범위와 한계를 근거와 함께 알려 주세요.",
  )
  ...
  await page.getByRole("textbox", { name: "의료 질문" }).fill(
    "산후 1년 동안 확인해야 할 경고 신호를 최신 근거와 함께 자세히 설명해 주세요.",
  )
  await page.getByRole("button", { name: "질문 보내기" }).click()
