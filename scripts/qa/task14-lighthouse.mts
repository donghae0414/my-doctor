...
const outputDirectory =
  process.env["TASK14_LIGHTHOUSE_OUTPUT"] ??
...
const remoteDebuggingPort = z.coerce
  ...
  .min(1_024)
  .max(65_535)
  .parse(process.env["TASK14_CHROME_CDP_PORT"] ?? 39_000 + (process.pid % 10_000))
...
type Scores = Readonly<Record<Category, number>>

const measuredScores: Record<Preset, Scores[]> = { desktop: [], mobile: [] }

function categoryScore(result: Awaited<ReturnType<typeof lighthouse>>, category: Category): number {
  if (result === undefined) return 0
  return Math.round((result.lhr.categories[category]?.score ?? 0) * 100)
}

function medianScore(preset: Preset, category: Category): number {
  const values = measuredScores[preset].map((scores) => scores[category]).sort((a, b) => a - b)
  return values[1] ?? 0
}
...
try {
  ...
  for (const preset of presets) {
    for (const run of measuredRuns) {
      ...
      measuredScores[preset].push(scores)
      const report = {
        metadata: {
          auditedUrl,
  ...
  const median: Readonly<Record<Preset, Scores>> = {
    desktop: {
      accessibility: medianScore("desktop", "accessibility"),
      "best-practices": medianScore("desktop", "best-practices"),
      performance: medianScore("desktop", "performance"),
      seo: medianScore("desktop", "seo"),
    },
    mobile: {
      accessibility: medianScore("mobile", "accessibility"),
      "best-practices": medianScore("mobile", "best-practices"),
      performance: medianScore("mobile", "performance"),
      seo: medianScore("mobile", "seo"),
    },
  }
  await writeFile(
    `${outputDirectory}/median.json`,
    `${JSON.stringify({ measuredScores, median, sourceHash }, null, 2)}\n`,
  )
  console.log(JSON.stringify({ event: "median", median }))
  if (presets.some((preset) => categories.some((category) => median[preset][category] !== 100))) {
    throw new RangeError("Lighthouse category median failed the 100 gate")
  }
} finally {
  await browser.close()
}
